...
DispatcherServlet 就是一个 Servlet，那Servlet 的初始化方法 init()在哪里，通过类图可知，可以查看 HttpServletBean 中的 init() 方法，进行 Servlet初始化。init()方法代码如下：
```
    @Override
	public final void init() throws ServletException {	    
		...
		try {
		    // 包装 DispatcherServlet 的 <init-param>标签的参数，包括contextConfigLocation 和 detectAllHandlerMappings 
...
protected WebApplicationContext initWebApplicationContext() {
        ...
        // web.xml 由 XmlWebApplicationContext 解析后转成 rootContext
...
```
	...
	public int loadBeanDefinitions(Resource... resources) throws BeanDefinitionStoreException {
		...
		int counter = 0;
		for (Resource resource : resources) {//循环xml 配置文件
		    //查看该方法,进入 XmlBeanDefinitionReader 类中的 loadBeanDefinitions 方法,如下图所示
			counter += loadBeanDefinitions(resource);  //解析 一个 xml文件
		}
		return counter;
	}
```
...
public NamespaceHandler resolve(String namespaceUri) {
        // 查看 getHandlerMappings() 方法 ,命名空间到应的处理类，spring初始化就加载了
...
```
	...
	public BeanDefinition parse(Element element, ParserContext parserContext) {
		...
		// 这个方法处理完毕后，扫描器中会加入需要扫描的注解和需要排除的注解 类型
		ClassPathBeanDefinitionScanner scanner = configureScanner(parserContext, element);
		
		// 查看该方法 俱体的扫描操作,俱体分析看下面
		// 解析相关注解的值，并将 beans 的名字放入集合 beanDefinitionNames  中 
		Set<BeanDefinitionHolder> beanDefinitions = scanner.doScan(basePackages);
		
		// 查看该方法，俱体分析看下面
		// 各种注解解析 如 @Autowired @Required 等
		registerComponents(parserContext.getReaderContext(), beanDefinitions, element);
...
protected ClassPathBeanDefinitionScanner configureScanner(ParserContext parserContext, Element element) {
		...
		//创建 扫描器，扫描包下的所有文件 ，同时也是spring @Controller、@Service 注解的注册支持，查看该方法
...
- 当xml 配置文件中配置了 use-default-filters="false" 时就不会加载 @Component 注解的所有子注解，如 @Component、@Controller、@Service 注解的类都不会扫描。
  use-default-filters="false"属性是专门和context:include-filter子标签一起使用，这样可以更加自由地指定哪些注解由扫描器扫描。其意思相当于：只扫描@xxx注解的标志的类。
...
protected Resource[] findPathMatchingResources(String locationPattern) throws IOException {
		...
		for (Resource rootDirResource : rootDirResources) {
			...
			// jar 文件的情况
			    ...
			    // .class 文件的情况 重点
			    // 查看该方法 doFindPathMatchingFileResources 方法，进入PathMatchingResourcePatternResolver 类
...
// 分析 PathMatchingResourcePatternResolver 类 doFindPathMatchingFileResources 方法
...
protected void doRetrieveMatchingFiles(String fullPattern, File dir, Set<File> result) throws IOException {
		...
		for (File content : dirContents) {
			...
			// 如果时文件，就加入 集合中,分析到 此时，回到 ClassPathBeanDefinitionScanner 类中的 doScan(basePackages)方法中
...
### 回到 ClassPathBeanDefinitionScanner 类中的 doScan(basePackages)方法中 分析注解 解析
```
if (candidate instanceof AnnotatedBeanDefinition) {
    AnnotationConfigUtils.processCommonDefinitionAnnotations((AnnotatedBeanDefinition) candidate);
	}
...
所有生成的beanDifinition对象都会注册缓存到map中，
然后会把beanName放到List里面去，beanDifinitionNames 这个list

### 到此为止 AbstractApplicationContext 类中的 obtainFreshBeanFactory() 方法分析完毕
代码片段为：
```
