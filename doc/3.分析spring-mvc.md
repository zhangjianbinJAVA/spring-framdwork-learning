...
protected void initHandlerMethods() {
		...
		for (String beanName : beanNames) {
			if (!beanName.startsWith(SCOPED_TARGET_NAME_PREFIX)) {
				...
				=====================================>>  isHandler(beanType) 方法 ，在 RequestMappingHandlerMapping类中
					@Override
				...
				if (beanType != null && isHandler(beanType)) {//只过滤是 @Controller 或者@RequestMapping 的类，因为只有 @Controller类才有　uri 的配置
...
protected ModelAndView invokeHandlerMethod(HttpServletRequest request,
	...
	public void invokeAndHandle(ServletWebRequest webRequest,
		...
		mavContainer.setRequestHandled(false); // 参数很重要
		try {
...
private List<HandlerMethodArgumentResolver> getDefaultArgumentResolvers() {
   ...
   // 查看RequestMappingHandlerAdapter 类中的构造器 ，在这里注册了默认的 messageConverters
...
private ModelAndView getModelAndView(ModelAndViewContainer mavContainer,
		...
		if (mavContainer.isRequestHandled()) {// 当为 true时，说明返回的不是视图，例如　返回json时
			return null;
		}
